	...
	public unsafe sealed partial class Fast
	{
		...
		public static byte[] FromHexString (string source)
		{
			...
			if (string.IsNullOrEmpty (source))
				return new byte[0];
			// you may change it to return null
					...
					fixed (byte* lowRef = fromHexTable)
						...
						fixed (byte* resultRef = result) {
							...
							while (*s != 0) {
								...
								if (// check for character > 'f'
								*s > 102 || // assign source value to current result position and increment source position
								// and check if is a valid character
								(*r = hiRef [*s++]) == 255 || // check for character > 'f'
								*s > 102 || // assign source value to "add" parameter and increment source position
								// and check if is a valid character
								(add = lowRef [*s++]) == 255)
									throw new ArgumentException ();
